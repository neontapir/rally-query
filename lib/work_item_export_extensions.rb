...
module WorkItemExportExtensions
  def self.included(ignored)
    WorkItem.include WorkItemExportExtensionMethods
  end

  module WorkItemExportExtensionMethods
    def actual_story_points
      raise 'Story points method missing' unless respond_to?(:story_points)
      @story_points
    end

    def state
      raise 'Current state method missing' unless respond_to?(:current_state)
